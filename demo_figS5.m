function demo_figS5
% Running this program is very time-consuming, taking around two days to
% complete. 
...
fun_SSA = @(CIRCA_PARA)fun_sobol_circadian_parameter(CIRCA_PARA,Par_ada, Par_decay, Par_produce, Par_transition, Par_halfmax, Par_Others, ...
    Par_promotion, Par_clock, Par_Clock_immune,Time_circadian_clock);


circa_para = [0.361903467, 0.306890098, 0.006076323, 0.003890361,...
    0.1649, 0.0558, 0.01];%(CRY-TNFa, ROR-TNFa, REV-il6, REV-il10, CB-CCL21, CB-S1PR1, REV-Th)
% circa_para = ones(1,7);%(CRY-TNFa, ROR-TNFa, REV-il6, REV-il10, CB-CCL21, CB-S1PR1, REV-Th)
% circa_para = [0.1649,0.361903467, 0.306890098, 0.006076323, 0.003890361,...
%     0.1649, 0.0558, 0.01];%(CRY-TNFa, ROR-TNFa, REV-il6, REV-il10, CB-CCL21, CB-S1PR1, REV-Th)
ub = circa_para*1.2;
lb = circa_para/1.2;
N =4;
[S, ST] = sobol_sensitivity(fun_SSA, lb, ub, N);
save SSA_K_LPS_5000_2.mat S ST
% load SSA_ZT10_0417_2025.mat S ST

param_names = {'K_{CRY}^{TNF\alpha}', 'K_{ROR}^{TNF\alpha}', 'K_{REV}^{IL6}','K_{REV}^{IL10}', 'K_{CB}^{CCL21}','K_{CB}^{S1PR1}','K_{REV}^{Th}'};
% param_names = {'K_{CB}^{APCm}','K_{CRY}^{TNF\alpha}', 'K_{ROR}^{TNF\alpha}',...
%     'K_{REV}^{IL6}','K_{REV}^{IL10}', 'K_{CB}^{CCL21}','K_{CB}^{S1PR1}','K_{REV}^{Th}'};
% param_names = {'\lambda_{CRY}^{TNF\alpha}', '\lambda_{ROR}^{TNF\alpha}', '\lambda_{REV}^{IL6}','\lambda_{REV}^{IL10}', '\lambda_{CB}^{CCL21}',...
%     '\lambda_{CB}^{S1PR1}','\lambda_{REV}^{Th}'};
plot_sobol_indices(S, ST, param_names)
end
function ADA_delta = fun_sobol_circadian_parameter(CIRCA_PARA, Par_ada, Par_decay, Par_produce, Par_transition, Par_halfmax, Par_Others, ...
    Par_promotion, Par_clock, Par_Clock_immune, Time_circadian_clock)

Par_halfmax(100) = CIRCA_PARA(1);
Par_halfmax(103) = CIRCA_PARA(2);
Par_halfmax(91) = CIRCA_PARA(3);
Par_halfmax(85) = CIRCA_PARA(4);
Par_Clock_immune(8) = CIRCA_PARA(5);
Par_Clock_immune(9) = CIRCA_PARA(6);
Par_Clock_immune(7) = CIRCA_PARA(7);

% Par_Others(43) = CIRCA_PARA(1);
% Par_Others(44) = CIRCA_PARA(2);
% Par_Others(45) = CIRCA_PARA(3);
% Par_Others(46) = CIRCA_PARA(4);
% Par_Others(47) = CIRCA_PARA(5);
% Par_Others(48) = CIRCA_PARA(6);

% Par_Others(35) = 1;
% Par_Clock_immune(14) = CIRCA_PARA(1);
% Par_halfmax(100) = CIRCA_PARA(2);
% Par_halfmax(103) = CIRCA_PARA(3);
% Par_halfmax(91) = CIRCA_PARA(4);
% Par_halfmax(85) = CIRCA_PARA(5);
% Par_Clock_immune(8) = CIRCA_PARA(6);
% Par_Clock_immune(9) = CIRCA_PARA(7);
% Par_Clock_immune(7) = CIRCA_PARA(8);
...
ADA_delta = (ada_ZT7(end) - ada_ZT19(end)) / ada_ZT7(end);
end
...
function [S, ST] = sobol_sensitivity(f, lb, ub, N)
    D = length(lb);
    lb = [lb,lb];
    ub = [ub,ub];
    % 生成Sobol序列样本
    sob = sobolset(2*D);
    R=[];
for i=1:N % 选取前nPop行被上下限空间处理后的样本
    r=sob(i,:);
    r=lb+r.*(ub-lb);
   R=[R; r]; 
end
...
YAB = zeros(N,D);
for i=1:N
    YA(i)=f(A(i,:));  %A矩阵对应的YA值
    YB(i)=f(B(i,:));%B矩阵对应的YB值
    for j=1:D
        YAB(i,j)=f(AB(i,:,j));%YAB矩阵对应的YAB值
    end
end
VarX=zeros(D,1);% S的分子
S=zeros(D,1);
